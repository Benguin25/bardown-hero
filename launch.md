...
Audio is enabled by default: the campaign and gameplay headers include an **SND ON/OFF** toggle. Bardown Hero uses original punchy arcade hockey effects for passes, boards, saves, goals, powerups, and countdowns, plus distinct snapshot, one-timer, curved-shot, screened-shot, and rebound-finish releases. The louder stadium-electronic music continues across campaign and gameplay transitions, ducks under the goal horn, stops when the app is backgrounded, and resumes on return. The setting is saved locally, and playback is configured to remain audible when the iPhone silent switch is on.
...
- Finish toward the net: the preview turns gold and identifies the high/low and left/right lane. Aim inside the red posts.
- Flick through the net to shoot instead of dragging onto a corner marker. Finish left or right inside the posts to choose that side. Follow through a little beyond the goal line for a low shot or farther toward the end boards for a high shot; the preview rises immediately to confirm the choice. A centered low flick deliberately tests the pads for a rebound. The goalie reads the puck's heading after a short reaction delay, slides with momentum, and reaches with the glove. Curved releases and quick one-timers can still wrong-foot that reaction, while gestures outside the posts remain misses.
...
Gameplay feel pass: passes, receiver pursuit, and the opening skate are faster, with a shorter guided rebound pause. Slow motion starts only for shots near the goal and their rebounds; passes through the same area remain at full speed. A receiver commits to an interception point and skates there to meet a good pass. Missed passes remain live and the closest reachable teammate gives chase. Lead passes have slightly more collection reach, and banks keep more speed after contact. Mega Curve retains its exaggerated bend with less sensitivity to uneven finger sampling. Flick shots use broad side and follow-through bands, while center shots remain aimed at the pads. Snapshots, one-timers, curves, screens, and rebound finishes now have distinct release feedback. Skaters turn smoothly instead of snapping to each new heading.
