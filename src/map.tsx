...
function TrophyStrip({ venue, earned, muted = false }: { venue: Venue; earned: number; muted?: boolean }) {
  const target = venue.count * 3, left = target - earned;
  return <View style={[m.trophy, muted && m.trophyMuted]}>
...
function LevelRow({ index, earned, state, onPress }: {
  ...
  const label = cleared ? (earned === 3 ? '3 / 3 ★ PERFECT' : `${earned} / 3 ★ · REPLAY`) : state === 'next' ? 'UP NEXT' : 'LOCKED';
...
export function CampaignMap({ progress, loaded, error, save, select, soundOn, toggleSound, profileName, playerNumber, openProfile, openAchievements, onHelp }: {
  ...
  const tapNode = (index: number, x: number, y: number) => {
    ...
    root.current?.measureInWindow((left, top) => setCard({ index, x: x - left, y: y - top }));
  };
  ...
  const sections = VENUES.slice().reverse().map(venue => {
    ...
    return <View key={venue.id} style={m.venue}
      ...
      {!!sectionHeights[venue.id] && <>
        <View style={[m.path, { top: 0 }]}><DashLine height={sectionHeights[venue.id]} color={behind ? c.ringIdle : c.pathIdle} /></View>
      ...
      <TrophyStrip venue={venue} earned={venueStars(progress, venue)} muted={behind} />
      {rows}
    </View>;
  });
...
export function LevelCard({ index, origin, viewport, progress, onClose, onPlay }: {
  index: number; origin: { x: number; y: number }; viewport: { width: number; height: number }; progress: Progress; onClose: () => void; onPlay: () => void;
}) {
  ...
  const translateX = grow.interpolate({ inputRange: [0, 1], outputRange: [origin.x - viewport.width / 2, 0] });
  const translateY = grow.interpolate({ inputRange: [0, 1], outputRange: [origin.y - viewport.height / 2, 0] });
  return <View style={s.fill}>
    ...
    <View style={m.cardWrap} pointerEvents="box-none">
      <Animated.View style={[m.card, { opacity: grow, transform: [{ translateX }, { translateY }, { scale }] }]}>
        ...
        <ScrollView style={m.cardScroll} contentContainerStyle={m.cardBody} showsVerticalScrollIndicator={false}>
...
export function VenueUnlocked({ venue, playerNumber, nextLevel, onEnter }: {
  ...
  return <SafeAreaView style={s.root}>
    ...
    <ScrollView style={m.unlockScroll} contentContainerStyle={m.unlockContent} showsVerticalScrollIndicator={false}>
...
const m = StyleSheet.create({
  ...
  rowTitleCleared: { color: c.ice400 },
  ...
  cardWrap: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, justifyContent: 'center', paddingHorizontal: 14, paddingVertical: 20, zIndex: 10, elevation: 10 },
  ...
  unlockScroll: { flex: 1 },
  unlockContent: { flexGrow: 1, paddingHorizontal: 20, paddingTop: 24, paddingBottom: 20 },
  unlockHead: { alignItems: 'center', paddingHorizontal: 6, marginTop: 24 },
  ...
  unlockCard: { marginTop: 22, borderRadius: radii.panel, backgroundColor: 'rgba(6,21,32,0.78)', borderWidth: 1, borderColor: 'rgba(255,197,49,0.34)', padding: 16, paddingHorizontal: 18, flexDirection: 'row', alignItems: 'center', gap: 14 },
